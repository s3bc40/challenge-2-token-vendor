...
const deployVendor: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  */
  ...
  const totalSupply = await yourToken.totalSupply(); // Side quest : losing assets if we do not use proper method 'totalSupply' ERC20
  await yourToken.transfer(vendorAddress, hre.ethers.parseEther(hre.ethers.formatEther(totalSupply)));
  // Transfer contract ownership to your frontend address
  await vendor.transferOwnership("0x815C72551613c8f170515bA914F4125faC85c7E4");
};
